...
/datum/admins/proc/populate_world(turf/T as turf)
	set name = "Spawn Dummy Survivors (Exodus)"
	set category = null
	set desc = "Spawns a number of clientless, offline survivors for testing Exodus mode scaling."

	if(!check_rights(R_SPAWN)) // Or a higher admin right
		return

	var/amount = tgui_input_number(user, "please enter the number of humans you want")
	...
	for(var/i in 1 to amount) {
		var/turf/spawn_turf = pick(GLOB.exodus_survivor_spawns).loc
		if(!spawn_turf) continue

		// Create the dummy mob
		var/mob/living/carbon/human/dummy/new_dummy = new(spawn_turf)
		...
		new_dummy.faction = survivor_job.faction
...
/datum/admins/proc/recalculate_exodus_threat()
	set name = "Recalculate Exodus Threat"
	set category = null
	set desc = "Recalculates Exodus threat thresholds and values based on the current survivor count. Run AFTER spawning dummies."
