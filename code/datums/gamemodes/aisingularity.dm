...
/datum/game_mode/aisingularity/post_setup()
	. = ..()
	if(bioscan_interval)
...
/datum/game_mode/aisingularity/process()
	if(round_finished)
		return PROCESS_KILL
...
/datum/game_mode/aisingularity/announce_bioscans(show_locations = TRUE, delta = 2, announce_humans = TRUE, announce_xenos = TRUE, send_fax = TRUE)
	TIMER_COOLDOWN_START(src, COOLDOWN_BIOSCAN, bioscan_interval)
...
/datum/game_mode/aisingularity/check_finished()
	if(round_finished)
		return TRUE
...
/datum/game_mode/aisingularity/declare_completion()
	. = ..()
...
/datum/game_mode/aisingularity/can_start(bypass_checks = FALSE)
	. = ..()
	if(!.)
		return
...
/datum/game_mode/aisingularity/pre_setup()
	. = ..()
	addtimer(CALLBACK(SSticker.mode, PROC_REF(map_announce)), 5 SECONDS)

///Announce the next map
/datum/game_mode/aisingularity/proc/map_announce()
...
/datum/game_mode/aisingularity/announce()
	to_chat(world, span_round_header("The current map is - [SSmapping.configs[GROUND_MAP].map_name]!"))

/datum/game_mode/aisingularity/attempt_to_join_as_larva(mob/dead/observer/observer)
	var/datum/hive_status/normal/HS = GLOB.hive_datums[XENO_HIVE_NORMAL]
	return HS.add_to_larva_candidate_queue(observer)


/datum/game_mode/aisingularity/spawn_larva(mob/xeno_candidate, mob/living/carbon/xenomorph/mother)
	var/datum/hive_status/normal/HS = GLOB.hive_datums[XENO_HIVE_NORMAL]
	return HS.spawn_larva(xeno_candidate, mother)

/datum/game_mode/aisingularity/proc/on_nuclear_diffuse(obj/machinery/nuclearbomb/bomb, mob/living/carbon/xenomorph/X)
	SIGNAL_HANDLER
...
/datum/game_mode/aisingularity/proc/on_nuclear_explosion(datum/source, z_level)
	SIGNAL_HANDLER
	planet_nuked = INFESTATION_NUKE_INPROGRESS
	INVOKE_ASYNC(src, PROC_REF(play_cinematic), z_level)

/datum/game_mode/aisingularity/proc/on_nuke_started(datum/source, obj/machinery/nuclearbomb/nuke)
	SIGNAL_HANDLER
	var/datum/hive_status/normal/HS = GLOB.hive_datums[XENO_HIVE_NORMAL]
...
/datum/game_mode/aisingularity/proc/play_cinematic(z_level)
...
/datum/game_mode/aisingularity/proc/do_nuke_z_level(z_level)
	if(!z_level)
		return
